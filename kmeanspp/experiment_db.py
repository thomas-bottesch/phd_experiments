...
class ExperimentDB():
  ...
  def add_experiment(self, control_params, params, res, exclude_keys_from_json=[]):
  ...
  def remove_field_from_results(self, fieldname):

    for i in range(self.next_algorithm_run_id):
      output_file_run = join(self.output_folder_local, self.run_identifier + "_" + str(i))
      output_file_run_json = output_file_run + ".txt"

      with open(output_file_run, 'rb') as f:
        (control_params, params, res) = cPickle.load(f)
      if fieldname in res:
        print(output_file_run)
        del res[fieldname]
      
      with open(output_file_run, 'wb') as f:
        cPickle.dump((control_params, params, res), f)
      
      if isfile(output_file_run_json):
        with open(output_file_run_json, 'wb') as f:
          json.dump(res, f, indent=4)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--function', type=str, default="do_kmeanspp", help='Path to the results of single algorithm executions')
  parser.add_argument('--delete_result_field', default=None, help='Path to the results of single algorithm executions')
  args = parser.parse_args()

  run_identifiers = ExperimentDB.get_identifiers(args.output_path, args.function)

  if args.delete_result_field is not None:
    for run_identifier in run_identifiers:
      db = ExperimentDB(args.output_path, args.function, run_identifier)
      db.remove_field_from_results(args.delete_result_field)
    sys.exit(0)
  ...
  for run_identifier in run_identifiers:
    ...
    for resid in db.get_algorithm_run_ids():
      ...
      if params['info']['algorithm'] in ["bv_yinyang"]:
        pass
