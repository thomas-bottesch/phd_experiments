...
from minibatch_best_params import result_evaluation_minibatch_best_params
from latex_plots_minibatch_best_params import create_plot
...
def do_minibatch_best_params(result_q, control_params, params):
    ...
    if 'pca' in info['algorithm']:
      ...
      # set the seed to a specific value to get reproducible random results
      np.random.seed(task['run'])
      ...
      # Simulate that we only have two batches to do the pca
      sampled_matrix = scipy_csr_matrix[np.random.choice(scipy_csr_matrix.shape[0], scipy_csr_matrix.shape[0]/10, replace=False), :]
      
      p.fit(sampled_matrix)
      # convert to millis
...
def do_evaluations(dataset_path, dataset_name, out_folder, params, clusters, truncated_svd_annz_percentage, bv_annz):
  ...
  algorithms = ["minibatch_kmeans", "bv_minibatch_kmeans", "pca_minibatch_kmeans"]
      ...
      for algorithm in algorithms:
        ...
        for r in range(3):
          ...
          if algorithm in slow_algorithms and dataset_name in huge_datasets:
            ...
            # the full kmeans
            params['calculate_kmeans']['task']['iteration_limit'] = 50
          else:
          ...
          if 'pca' in algorithm:
            for perc in truncated_svd_annz_percentage:
              ...
              try:
                evaluate("minibatch_best_params", control_params, params_specific['calculate_kmeans'], do_minibatch_best_params, out_folder)
              except:
...
if __name__ == "__main__":
  ...
  parser.add_argument('--output_path_latex_append_folder', type=str, default="minibatchkmeans")
  ...
  datasets_to_ignore_in_stage1 = {'caltech101': None,
                                  ...
                                  'mnist800k': None,
                                  'e2006': None}
  result_data = result_evaluation_minibatch_best_params(out_folder, output_path_latex, ignore_datasets=datasets_to_ignore_in_stage1)
  ...
  result_data = result_evaluation_minibatch_best_params(out_folder, output_path_latex, remove_incomplete=True)
...
for name, table_type in tabs:
  create_table(output_folder=output_path_latex,
              plot_name="plot-%s-comparison-minibatchkmeans" % name,
              pdata=copy.deepcopy(result_data),
              best_params=best_params,
              algs_to_display={'pca_minibatch_kmeans': None,
                               'bv_minibatch_kmeans': None,
