        ...
        if reference_no_iterations is None:
          ...
          for run in reference_no_iterations:
            ...
            if result_data[base_alg]['no_iterations'][run] != reference_no_iterations[run]:
              print("need to approximate minibatch_kmeans ds=%s, no_clusters=%s, run=%s, iterations=%s"
...
def remove_incomplete_data(result_data):
  ...
  for ds in result_data:
    ...
    for no_clusters in result_data[ds]['results']:
      if remove_where_kmeans_not_complete(result_data[ds]['results'][no_clusters], ds, no_clusters, 'minibatch_kmeans'):
...
def result_evaluation_minibatch_best_params(out_folder, out_folder_csv, remove_incomplete=False, ignore_datasets={}):
  ...
  for run_identifier in run_identifiers:
    ...
    for resid in db.get_algorithm_run_ids():
      ...
      for descr in ['iteration_durations', 'iteration_changes', 'iteration_wcssd', 'duration', 'no_iterations']:
        if descr not in result_data[ds]['results'][no_clusters][alg]:
          result_data[ds]['results'][no_clusters][alg][descr] = OrderedDict()
      
      for descr in ['iteration_durations', 'iteration_changes', 'iteration_wcssd', 'duration']:
