...
column_titles = {TABLE_TYPE_DURATION: "Speed-up relative to \\kmeans",
                 TABLE_TYPE_MEMORY: "\\specialcell[c]{Memory consumption \\\\ relative to \\kmeans}"}
      ...
      for no_clusters in pdata[dataset]['results']:
        for alg in pdata[dataset]['results'][no_clusters]:
          ...
          if bp is None:
            ...
            continue
          for measurment in ['duration', 'mem_consumption']:
...
def create_table(output_folder=None,
    ...
    algs["nc_kmeans"] = "nc \\\\ \\kmeans{} "
    algs["pca_kmeans"] = "$\\varphi_{p}$ \\\\ \\kmeans{} \\\\ \scriptsize (t=%.2f)" % best_params['pca']
    algs["bv_kmeans"] = "$\\varphi_{\\B}$ \\\\ \\kmeans{} \\\\ \scriptsize (b=%.2f)" % best_params['bv']
    algs["pca_elkan"] = "$\\varphi_{p}$ \\\\ Elkan \\\\ \scriptsize (t=%.2f)" % best_params['pca']
    algs["bv_elkan"] = "$\\varphi_{\\B}$ \\\\ Elkan \\\\ \scriptsize (b=%.2f)" % best_params['bv']
    algs["pca_yinyang"] = "$\\varphi_{p}$ \\\\ Yinyang \\\\ \scriptsize (t=%.2f)" % best_params['pca']
    algs["bv_yinyang"] = "$\\varphi_{\\B}$ \\\\ Yinyang \\\\ \scriptsize (b=%.2f)" % best_params['bv']
