...
\\def\\pathpicturesparamsearch{{./pictures/}}
...
"""
      \\begin{{figure}}
        \\centering
        \\input{{\\pathpicturesparamsearch {filename}}}
...
def make_time_percent_reference(pdata, speedup=False):
    
    algorithms_to_delete = {"yinyang":None, "elkan":None, "kmeans":None, "nc_kmeans":None}
    reference_times = {}
    for dataset in pdata:
      for no_clusters in pdata[dataset]['results']:
        for alg in pdata[dataset]['results'][no_clusters]:
          if alg not in ["yinyang", "elkan", "kmeans"]:
            continue
          data_array = []
          for run in range(3):
            reference_times[(dataset, no_clusters, alg, run)] = pdata[dataset]['results'][no_clusters][alg]['duration'][run][0]
          if "yinyang" not in alg and "elkan" not in alg:
    ...
    for (dataset, no_clusters, alg, run) in reference_times:
      reference_times[(dataset, no_clusters, alg, run)] = reference_times[(dataset, no_clusters, "kmeans", run)] / reference_times[(dataset, no_clusters, alg, run)] 
...
{tables}
  ...
  height=6cm, xlabel={x_label}, ylabel=speedup vs. lloyd, grid=both, grid style={{line width=.1pt, draw=gray!10}},
...
def create_plot_line(color, variable_name):
  ...
  """
  # Plus and minus do not work well together with the latex legend. Since they are separate plots. We would have to jump over one of them.
  tmpl = "\\addplot[mark=, color={color},line width=1.5pt] plot [error bars/.cd, y dir = plus, y explicit] table[x={{0}},y={{1}}, y error expr={{\\thisrow{{4}} - \\thisrow{{1}}}}] {{\\{variable_name}}}; \\\n"
  tmpl += "\\addplot[mark=, color={color},line width=1.5pt] plot [error bars/.cd, y dir = minus, y explicit] table[x={{0}},y={{1}}, y error expr={{\\thisrow{{1}} - \\thisrow{{3}}}}] {{\\{variable_name}}};"
  """
...
def determine_best_params(pdata_new):
    ...
    best_params = {'bv': decision_tuples_bv[0][1],
                   'pca': decision_tuples_pca[0][1]}
    
    return best_params

def determine_difference_from_best_params(pdata_new, best_params):

    speedups = {}

    for dataset in pdata_new:
      if dataset not in speedups:
        speedups[dataset] = {}
      for alg in pdata_new[dataset]:
        if alg not in speedups[dataset]:
          speedups[dataset][alg] = {}

        for no_clusters in pdata_new[dataset][alg]:
          if no_clusters not in speedups[dataset][alg]:
            speedups[dataset][alg][no_clusters] = {'speedups': {}}

          tuples = []
          for (param, speedup, _, _, _) in pdata_new[dataset][alg][no_clusters]:
            tuples.append((speedup, param))
          tuples.sort(reverse=True)
          max_speedup, p = tuples[0]
          speedups[dataset][alg][no_clusters]["param_max_speedup"] = p
          speedups[dataset][alg][no_clusters]["max_speedup"] = max_speedup

          for (param, speedup, _, _, _) in pdata_new[dataset][alg][no_clusters]:
            speedups[dataset][alg][no_clusters]['speedups'][param] = (speedup/max_speedup, speedup)
          

    print("Comparison of speed-ups from default parameters %s compared to the optimal parameters:" % str(best_params))
    all_res = []
    for dataset in sorted(speedups):
        for alg in sorted(speedups[dataset]):
          if 'pca' in alg:
            typ = 'pca'
          if 'bv' in alg:
            typ = 'bv'

          for no_clusters in sorted(speedups[dataset][alg]):
            param_max_speedup = speedups[dataset][alg][no_clusters]["param_max_speedup"]
            max_speedup = speedups[dataset][alg][no_clusters]["max_speedup"]
            scaled_speedup, best_params_speedup = speedups[dataset][alg][no_clusters]['speedups'][best_params[typ]]
            bp_param = best_params[typ]
            all_res.append((scaled_speedup, dataset, alg, no_clusters, bp_param, best_params_speedup, param_max_speedup, max_speedup))

            print('{:>10} {:>12} {:>5} {:>4} {:>4} {:>6} {:>5} {:>6} '.format(dataset, alg, str(no_clusters), str(bp_param),
                                                                      "%.2f" % scaled_speedup, "%.2f" % best_params_speedup, param_max_speedup, "%.2f" % max_speedup))

    print("Cases where the default params perform worst:")
    for scaled_speedup, dataset, alg, no_clusters, bp_param, best_params_speedup, param_max_speedup, max_speedup in sorted(all_res)[:20]:
        print('{:>10} {:>12} {:>5} {:>4} {:>4} {:>6} {:>5} {:>6} '.format(dataset, alg, str(no_clusters), str(bp_param),
                                                                  "%.2f" % scaled_speedup, "%.2f" % best_params_speedup, param_max_speedup, "%.2f" % max_speedup))

    scaled_speed_ups = np.array([(100.0 - (x[0] * 100)) for x in all_res])
    print("Default params are on average %.1f%% (+- %.1f%%) slower than optimal parameters" % (np.mean(scaled_speed_ups), np.std(scaled_speed_ups)))
...
def create_plot(output_folder=None,
    ...
    #make_time_percent(pdata)
    reference_times = make_time_percent_reference(pdata, speedup=True)
    ...
    if only_best_params:
      return best_params
    determine_difference_from_best_params(pdata_new, best_params)
