...
def delete_avira_10_iterations(output_path, function):
  ...
  for run_identifier in run_identifiers:
    ...
    for resid in db.get_algorithm_run_ids():
      ...
      if params['info']['algorithm'] in ["kmeans"] and params['info']['dataset_name'] in ["avira201"] and params['task']['no_clusters'] == 10000 and params['task']['iteration_limit'] == 10:
        ids_to_delete[resid] = None

    db.remove_experiments_with_run_ids(ids_to_delete)

def delete_huge_dataset_iteration_not_10(output_path, function):
  run_identifiers = ExperimentDB.get_identifiers(output_path, function)
  ...
  for run_identifier in run_identifiers:
    ...
    for resid in db.get_algorithm_run_ids():
      (control_params, params, res) = db.get_experiment_result_from_run_id(resid)
      if params['info']['algorithm'] in slow_algorithms and params['info']['dataset_name'] in huge_datasets and params['task']['iteration_limit'] != 10:
        ids_to_delete[resid] = None
...
def delete_pca_elkan_mediamill_014(output_path, function):
  run_identifiers = ExperimentDB.get_identifiers(output_path, function)
  for run_identifier in run_identifiers:
    db = ExperimentDB(output_path, function, run_identifier)
    ids_to_delete = {}
    for resid in db.get_algorithm_run_ids():
      (control_params, params, res) = db.get_experiment_result_from_run_id(resid)
      if params['info']['algorithm'] in ["pca_elkan"] and params['info']['dataset_name'] in ["mediamill"] and params['task']['no_clusters'] == 100 and res["info"]["truncated_svd_annz_percentage"] == "0.14" and res["no_iterations"] == 153:
        ids_to_delete[resid] = None
...
if __name__ == "__main__":
  ...
  parser.add_argument('--function', type=str, default="do_best_params", help='Path to the results of single algorithm executions')
  args = parser.parse_args()
  
  delete_avira_10_iterations(args.output_path, args.function)
  delete_huge_dataset_iteration_not_10(args.output_path, args.function)
  delete_pca_elkan_mediamill_014(args.output_path, args.function)